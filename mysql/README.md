...
| 存储引擎 | 特点                                                         | 备注                                                         |
| -------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| InnoDB   | 1. 支持事务<br>2. 支持行级锁<br>3. 当插入数据时，默认会根据主键进行排序，若未指定主键则会自动生成一个row_id | 如果应用程序支持事务或需要执行大量的INSERT或UPDATE操作，则应该使用InnoDB |
...
* 数据会存储在硬盘上，在真正需要处理数据时，首先会把数据加载到内存中去。当进行查表时，InnoDB并不是一行一行的读取，InnoDB中的数据大小单位叫`页`。 在mysql中可以使用如下命令查看页大小: **SHOW GLOBAL STATUS like 'Innodb_page_size';**
...
* 通过上述内容可以知道，InnoDB在存储数据时默认会按照主键排序，而在此过程中，mysql还做了一件事，就是建立了`聚簇索引`。`聚簇索引`是一棵**B+树**。我们通过**[https://www.cs.usfca.edu/~galles/visualization/BPlusTree.html](https://www.cs.usfca.edu/~galles/visualization/BPlusTree.html)**网站，根据**t1**表中的主键构建了这样的一棵**B+树**，如下所示：

  ![聚簇索引B+树](./聚簇索引.png)

  可以看到，叶子节点中存储了`t1`表中所有的主键，其实在MySQL构建这棵树中，它会存储其他的数据结构，于是，针对上述的数据结构，我重新绘制了一幅图：![聚簇索引2](./聚簇索引2.png)

   可以看到，每一个主键内部维护了每一行剩下的数据，也就是当我们执行如下sql时：
...
* 假设我们要为bcd三个字段创建一个**联合索引**。在创建联合索引之前，我们先统计下bcd字段的所有可能出现的值：**111、222、322、311、235、644、455、888、219、742、**，排序后就是`111 > 219 > 222 > 235 > 311 > 322 > 455 > 644 > 742 > 888`。同上，我们使用现有的工具[https://www.cs.usfca.edu/~galles/visualization/BPlusTree.html](https://www.cs.usfca.edu/~galles/visualization/BPlusTree.html)来构建b+树
  ...
  由上可知，在bcd索引中，每个节点内部都维护了当前数据对应的主键。之所以叫做**辅助索引**，就是因为在mysql中，最终都会通过主键去找数据(因为完整的数据都是存在聚簇索引对应的B+树中，如果走了**覆盖索引**，则不需要回表了)。而我们自定义的索引中，内部维护了当前数据对应的主键，所以最后还会根据这个主键去走聚簇索引。**注意: 这里的每一个节点(数据页)被加载一次就会产生一次IO(试情况而定，总而言之MySQL每次IO操作都会携带16字节的数据，此时有可能包含好几个叶子节点，这取决于每个叶子节点的大小)，之前我们说过，InnerDB引擎中的数据单位为页，在图中可能没有体现出来**。上述所说的`回表`的概念就是：在辅助索引确认的情况下，还要根据辅助索引各节点中对应的主键到`聚簇索引`中去找对应的数据（举个例子：我要查b=3, c=2, d=2的数据中的d字段，对应的SQL语句就是：**SELECT e FROM t1 WHERE b = 3 AND c = 2 AND d = 2**，此时的查询我们虽然会使用bcd的组合索引，但是由于bcd的叶子节点存储的是主键的id，并没有e这个字段的信息，此时就会进行回标操作，需要根据id到聚簇索引中找e字段的值）。
...
* 上面说到了，**有索引能走的情况下，至于最后到底走不走还是由mysql的查询优化器决定的**，下面我们来了解下mysql的查询优化器
...
* 举个例子：拿转账业务来说，A向B转账100元，如果事务提交了，A账户扣100元，B账户加100元。如果事务回滚了，A账户的钱没有动，B账户的钱也没有动。在这个例子中，我们看到了**原子性（两个操作要么同时成功要么同时失败）、一致性（不管是操作成功还是失败，A、B账户的钱都是保持业务一致的）、持久性（数据是落地到硬盘的，比较持久）**。唯一一个没有关联的就是隔离性，隔离性的话得用两个业务来做处理，比如A向B转账、A也向C转账，这两个业务是不互相影响的，这就是隔离性。
...
* 当mysql中的事务不是非自动提交时。假设我们使用**begin或者start transaction**开启一个事务而没有执行`commit`或`rollback`操作。当我们执行一些特定的操作时，会触发**隐式提交**机制。具体触发的操作如下：
...
* **INSERT: 会先加隐式锁来保护这条新插入的记录，来保证这条记录不会被其他事务给访问**
  ...
  比如：事务A正在插入数据，还没有提交事务（单据主键id为1000），此时另外一个线程2来更新id=1000的数据。此时线程2会阻塞，因为事务A已经为id=1000的数据添加一把隐式锁了。线程2需要事务Acommit或者回滚后才会解除阻塞。
  ```
...
* **UPDATE：会先给记录加x锁，再直接对记录进行修改**
  ...
  15567004	2	3	4	5
  5	2	3	5	3
  96547	2	79174	95868	41814.301015636236
  
  当我执行如下sql语句时（开启了事务A，还没提交）
    BEGIN;
    UPDATE t1 SET b = 2 WHERE b = 2;
  此时MySQL会锁住所有b=2的行，如果此时我们有另外一个事务B尝试更新 a = 15567001的记录，比如下面这条sql:
  UPDATE t1 SET c = 3 WHERE a = 15567004;
  此时事务B必定挂起，需要等事务A操作完毕。
  但是如果事务B尝试更新a = 111的记录，比如下面这条sql：
  UPDATE t1 SET c = 3 WHERE a = 111; 
  这条sql是能执行成功的，因此没有对a = 111的记录上锁。
  
  但是要注意，如果b = 2的数据太多，会导致锁表，最终可能连UPDATE t1 SET c = 3 WHERE a = 111; 的操作也无法执行成功。
  ```
...
* **读已提交(READ COMMITTED)事务级别的行锁**
  ...
  1. 对`主键`所处的行进行锁定
     ...
     `结论：当加了写锁的sql语句走了聚簇索引(主键)时，锁住的只是一行`
...
* **REPEATABLE READ可重复读事务级别的行锁**
  ...
  3. 不走任何索引的情况 ---- 不同于**READ COMMITTED的事务级别**，在此情况下，`此事务级别将全表进行锁定`
