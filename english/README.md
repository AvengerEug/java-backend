...
| 单词      | 读音         | 含义                 | 举例                                                         | 翻译                                         |      |
| --------- | ------------ | -------------------- | ------------------------------------------------------------ | -------------------------------------------- | ---- |
| optimized | ['ɑptɪ.maɪz] | 乐观；最佳化；最优的 | highly optimized protocol                                    | 高性能协议                                   |      |
| obvious   | -            | 明显的；显而易见的； | The advantage of using a POJO in your ChannelHandler is **obvious** | 在channelHandler使用POJO来的优点是显而易见的 |      |
...
| 单词               | 读音             | 含义         | 举例                                                         | 翻译                                                         |      |
| ------------------ | ---------------- | ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ---- |
| precisely tailored |                  | 量身定制     | You could even want to design and implement a whole new protocol that is precisely tailored to your need | 你甚至可以去设计并实现一整个正是你需要的新协议               |      |
| proprietary        | [prə'praɪə.teri] | 专有的       | A legacy proprietary protocol                                | 一个传统的专有协议                                           |      |
| prefer             | [prɪ'fɜr]        | 首选；更喜欢 | To download the right version of JDK, please refer to your preferred JDK vendor's web site. | 为了下载正确的JDK版本，请优先参考JDK供应商网站               |      |
| perform            | -                | 完成；表现   | ChannelFuture represents any request operation might not have been **performed** yet because all operations are async in netty。 | channelFuture代表着任何一个请求操作都还没有被完成，因为在netty中，所有的操作都是亦不过的。 |      |
| packets            | -                | 数据包       | Unfortunately, the buffer of a stream-based transport is not a queue of **packets** but a queue of bytes | 不幸的是，基于字节流传输的缓冲区不是一个数据包队列，而是一个字节队列。 |      |
...
| 单词         | 读音        | 含义                   | 举例                                                         | 翻译                                                         |      |
| ------------ | ----------- | ---------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ---- |
| retrieve     | [rɪ'triv]   | 检索；收回；重新得到   | retrieve information from web server                         | 从一个web服务器中检索信息                                    |      |
| realize      | [ˈriəˌlaɪz] | 认识到；了解到；体会到 |                                                              |                                                              |      |
| right away   | -           | 立即；马上             | You will be able to write a client and a server on top of Netty right away when you are at the end of this chapter | 当你读完章节的末尾后，你能够在Netty上编写客户端和服务端      |      |
| refer        | [rɪ'fɜ:(r)] | 参考；查阅             | To download the right version of JDK, please refer to your preferred JDK vendor's web site. | 为了下载正确的JDK版本，请优先参考JDK供应商网站               |      |
| reaches      | -           | 到达                   | Decreases the reference count by 1 and deallocates this object if the reference count reaches at 0 | 引用减少1。当引用数量达到0时，则解除分配。                   |      |
| represents   | -           | 代表；                 |                                                              |                                                              |      |
| respectively | -           | 分别                   | The reader index and the writer index represents where the message starts and ends respectively. | 这个读下标与写下标之间的区域代表着消息的起始位置和结束位置。 |      |
| reusable     | -           | 可复用的；             |                                                              |                                                              |      |
| revise       | [rɪ'vaɪz]   | 修正；修订；校正       |                                                              |                                                              |      |
...
| 单词            | 读音             | 含义               | 举例                                                         | 翻译                                                         |      |
| --------------- | ---------------- | ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ---- |
| scale           | [skeɪl]          | 缩放；扩展性；     | However, a general purpose protocol or its implementation sometimes does not scale very well | 然而，一个通用的协议的实现的扩展性有事不是很好               |      |
| special purpose | -                | 单一用途；特殊目的 | a highly optimized protocol implementation that is dedicated to a special purpose | 一个高性能协议的实现需要专注于某一个用途                     |      |
| sacrificing     | ['sækrɪ.faɪs]    | 牺牲               | What matters in this case is how quickly we can implement that protocol while not sacrificing the stability and performance of the resulting application. | 重要的是，我们如何快速的实现这个协议，并且不牺牲它的稳定性同时又具备高性能。 |      |
| stability       | [stə'bɪləti]     | 稳定性             | 同sacrificting                                               | 同sacrificting                                               |      |
| simplifies      | ['sɪmplɪ.faɪ]    | 简化               | Netty greatly simplifies and streamlines network programming | netty极大简化了网络编程                                      |      |
| suffer          | ['sʌfər]         | 忍受；受苦         | suffer from x                                                | 从x出忍受痛苦                                                |      |
| separation      | [.sepə'reɪʃ(ə)n] | 分离               | clear separation                                             | 明确分离                                                     |      |
