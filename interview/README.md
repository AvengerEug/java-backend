...
* 首先Dubbo在启动的过程中，会扫描我们的@Service注解，Dubbo认为它就是一个需要被暴露的服务。然后会将当前的服务信息与ApplicationConfig、ProtocolConfig、RegistryConfig等信息一起封装成一个叫ServiceBean的类。这个是一个spring的bean，它内部实现了Spring的initailingBean和applicationListener的扩展点。InitionlizeBean是在Bean初始化后的一个回调，在这里面会做一些校验和基础信息的配置。而applicationListener是spring的一个事件驱动机制，它内部主要监听的是spring的contextRefreshEvent，即上下文刷新事件。而Dubbo的服务暴露主要是在此处开始的，在服务暴露的过程中，会根据当前服务的配置来决定是立即暴露还是延迟暴露。不管是立即暴露还是延迟暴露，最终都会执行到服务ServiceConfig的doExport方法，在这个方法中，Dubbo需要根据当前服务实现了哪些协议来挨个暴露，在这里会用到Dubbo的自适应扩展机制，即在调用protocol.export方法时，会根据URL中内部参数key为protocol的值来决定使用具体的协议进行服务导出。在服务导出的过程中，底层会开启一个netty服务器，并将当前服务的一些uri与服务器进行绑定，绑定完了之后，还会将信息添加到注册中心中去，至此，Dubbo的服务暴露算是基本完成了。
...
* 通常，我们在某个bean中的某个属性中添加@Renference注解。并且拥有依赖注入的功能。其底层主要是因为Dubbo向Spring注册了一个叫ReferenceAnnotationBeanPostProcessor的后置处理器，这个处理器会识别每个bean内部的每一个被@Renference注解标识的属性。进而包装成ReferenceBean类型，ReferenceBean实现了FactoryBean接口，就是因为FactoryBean内部的getObject方法触发了服务引用的过程。在服务引用中，会从注册中心拉取对应的服务提供者的信息，并缓存到本地。缓存到本地之后，第二部就要创建代理对象了，在代理对象的内部逻辑中，会封装成invoker，而invoker内部还会包含一些Cluster类似服务治理相关的功能，这些细节都在代理对象内部，对于我们的调用者是透明的，除此之外，因为是RPC调用，内部还会初始化netty的客户端，后续发起RPC调用时底层都是使用这个客户端调用的。
...
* 有五种基础数据结构string、list、map、set、zset，除此此外还有hypeloglog、geo、bitmap。通常，我们可以充分利用这些数据结构，比如list可以利用它的特殊api实现堆、栈、队列的功能。而map的话，我们可以存储一些对象的机构、购物车信息。set和zset的话，我们可以用来去重、生成随机数、取交集、差集、并集等等。
...
* 此时可以使用Redis的redis-cluster集群模式，通常每一个master节点会搭配多个slave节点。并且每个master阶段负责的职责是不一样的。在这种集群模式下，它结合了主从复制、哨兵模式的特点，并且拥有特殊的hash算法来支撑海量数据。同时，这种集群模式下，也可以满足高并发、高可用的特性。
...
* 有。在我们支付中心中，有两个操作依赖于支付成功的支付单，分账和退货。因为我们的支付是担保交易的，用户支付成功后钱进入的是担保账户。而分账就是要分给指定的经销商，退货则是将钱退给用户。而我们的分账和退货业务的入口都是mq，这就会有问题啦，假设消息的发送顺序是这样的：分账、退货。但最终落到消费者处理的时候，有可能退货的消息先接收到，先执行了退货的逻辑，随后又执行分账的逻辑。这个时候就出现了分账、退货两个消息的处理顺序不一致的情况了，这个时候以后两个解决方案：1、因为分账和退款都是操作支付单的，我们可以对支付单上锁。2、或者将分账和退货的消费者设置为同一个，并且只能为一个消费者。

### 5.3、如何保证消息必达？

* 可以从两个方向去考虑，第一个是客户端发送消息到mq服务器的过程，这个阶段有些MQ（比如RabbitMQ）支持消息持久化。第二个就是保证消息服务器发送消息到消费者的过程了，这个过程的话可以有两个机制来保证，消费者可以开启ack模式，或者直接落地到应用的库里。